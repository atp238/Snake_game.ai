...
def main_menu():
    screen = pygame.display.set_mode((820, 720))
    pygame.display.set_caption('Snake Game Menu')

    # Tải hình ảnh nền
    background_image = pygame.image.load('OIG3.jpg')
    background_image = pygame.transform.scale(background_image, (820, 720))  # Đảm bảo kích thước chính xác
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                mouse_pos = event.pos
                # Kiểm tra vùng nhấp chuột cho "Chơi tự do"
                if 312 <= mouse_pos[0] <= 512 and 230 <= mouse_pos[1] <= 270:
                    game = SnakeGame(screen)
                    human_play(game)
                # Kiểm tra vùng nhấp chuột cho "AI chơi"
                elif 312 <= mouse_pos[0] <= 512 and 310 <= mouse_pos[1] <= 350:
                    ai_play()
                # Kiểm tra vùng nhấp chuột cho "Thoát"
                elif 312 <= mouse_pos[0] <= 512 and 360 <= mouse_pos[1] <= 400:
                    pygame.quit()
                    sys.exit()
...
def ai_play():
    ...
    # Khởi tạo mô hình và các tài nguyên cần thiết trước khi vòng lặp bắt đầu
